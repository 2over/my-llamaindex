...
def format_context_fn(**kwargs):
    ...
    context_list = [c.strip() for c in context_list if c.strip()] # 修复源代码中的bug
...
姓名: CoffeeBaby
身份证：123456798123456
这项工作中，我们开发并发布了Llama 2, 这是一组经过预训练和微调的大型语言模型(LLM), 其规模从70亿到700亿个参数不等。
我们经过微调的LLM成为Llama 2-Chat，针对对话用力进行了优化
在我们测试的大多数基准测试中，我们的模型都优于开源聊天模型，并且根据我们对有用性和安全性的人工评估，他们可能是闭源模型的合适替代品
"""

# 用户问题改写
fmt_promt = prompt_tmpl.format(
