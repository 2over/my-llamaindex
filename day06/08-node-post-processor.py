...
# 使用本地的重排序模型进行重拍
reranker = SentenceTransformerRerank(model=r"BAAI/bge-reranker-large", top_n=2)
...
# 创建timeWeightedPostProcessor
# TimeWeightedPostProcessor是LlamaIndex中的一个后处理器，用于根据文档节点的时间戳进行加权排序或过滤，以优先考虑更新更近、时间更相关的内容
# 本质还是会先按照语义搜索，尽管你有一个最新的文档，但是如果内容和问题相差太大也是和最终检索的文档排序有影响的
time_postprocessor = TimeWeightedPostprocessor(
    time_decay=0.5, # 控制文档的新旧信息衰减速度。值越大，越快忽略旧的内容
    top_k=3 # 最多返回3条
)
...
query_engine = RetrieverQueryEngine.from_args(
    retriever=retriever,
    node_postprocessor=[time_postprocessor],
)
