...
#         # 将keyword_nodes中的数据变成{"1": node, "2": node}
...
class CustomRetriever(BaseRetriever):
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        """Retrieve nodes given query"""
        # 分别从两个引擎中检索相关的节点(卡片)
        vector_nodes = self.__vector_retriever.retrieve(query_bundle)
        keyword_nodes = self.__keyword_retriever.retrieve(query_bundle)

        # 提取两组结果的Node ID, 方便做数学集合运算 {"1", "2", "3", "4", "5", "6"}
        vector_ids = {n.node.node_id for n in vector_nodes}
        keyword_ids = {k.node.node_id for k in keyword_nodes}

        # 创建一个字典， 把ID和节点对象关联起来，方便通过ID召回对象: 去重后的上下文
        combined_dict = {n.node.node_id: n for n in vector_nodes} # 将vector_nodes中的数据变成{"1": node, "2":node}
...
print("="* 50)
print( keyword_index.index_struct.table)
# 定义自定义检索器
